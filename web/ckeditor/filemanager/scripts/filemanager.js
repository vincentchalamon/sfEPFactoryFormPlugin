...
var setDimensions = function(){
	...
	$('#splitter, #filetree, #fileinfo, .vsplitbar').height(newH);
        $('#fileinfo').width($("#splitter").width()-$("#filetree").width()-$("#vsplitbar").width())
          .resize(function(){
            $(this).width($("#splitter").width()-$("#filetree").width()-$("#vsplitbar").width())
          });
}
			...
			// use FCKEditor 2.0 integration method
                        if($.urlParam('FCKEditor')){
                              if(data['Properties']['Width'] != ''){
                                ...
                                $("#" + $.urlParam('returnId'), window.opener.document).val(data['Path']);
                        }
		}
